...
class CalculationTranslator(NodeTranslator):
    ...
    @staticmethod
    def get_files_list(nobj, dirobj=None, files=None, prefix=None):
        """
        Get list of all files present in given directory.

        :param nobj: node object
        ...
        if dirobj:
            flist = nobj.list_objects(dirobj)
        else:
            flist = nobj.list_objects()
        ...
        for fname, ftype in flist:
            ...
            if ftype == FileType.FILE:
                ...
                CalculationTranslator.get_files_list(nobj, fname, files, prefix + [fname])
        return files
